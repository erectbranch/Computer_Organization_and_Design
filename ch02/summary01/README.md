...
> 1101을 shift left하면 1010이 된다.

> slli(shift left logical immediate). immediate는 position을 얼마나 Shift할지를 나타낸다.

- `slli`는 $2^{i}$ 만큼 곱하는 연산이다.

- `srli`는 $2^{i}$ 만큼 나누는 연산이다.(오직 unsigned에서만 가능하다.)

    > 예를 들어 signed integer라면, 10000(-16)에 $2^{2}$ 만큼 나누면 00100(+4)가 되는 식으로 전혀 다른 값이 나오게 된다.

immediate가 6bit인 이유는, 그보다 더 많이 shift하면 원래 있는 수를 모두 shifting해버리고 0만 남아버리기 때문이다.
...
64bit에서 오직 관심 있는 구간만 masking하고, 나머지는 모두 0으로 clear
...
condition이 true일 때 **branch**는 labeled instruction으로 jump한다.(**taken**) false일 경우 sequential하게 continue한다.(**not-taken**)

> labeled instruction: 아래 예시의 Loop, Exit처럼 label된 instruction. label은 32bit 안에 instruction들의 location(memory에서 instruction이 있는 address)을 담고 있다.
...
이를 RISC-V code로 compile하면

> i==j를 neq(not equal, i $\neq$ j)로 check하고 있는 점에 유의하자. (`beq ? ? loop body`로 하면, 바로 다음의 instruction부터는 loop body를 분리해서 다른 곳에 위치시켜야 하기 때문이다.)

```
      bne x22, x23, Else     // x22, x23이 같지 않다면 Else로 jump
      add x19, x20, x21
...
- i(offset)는 x22, k는 x24, address는 x25 register에 저장되어 있다고 하자.

> offset이 constant가 아니라는 점이 관건이다. `ld x5 {}(x25)`에서 <U>{} 부분이 constant(immediate)여야 하므로</U>, offset을 구하는 instruction들이 필요하다.

이를 RISC-V code로 compile하면

> 8을 곱해주는 이유는, 암묵적으로 long으로 선언되기 때문에, offset에 8을 곱해줘야 한다.

```
Loop:  slli x10, x22, 3     // 8을 곱해주기 위해 shift left을 3 bit 적용
       add  x10, x10, x25
       ld   x9, 0(x10)      // offset이 x9에 load된다.
       bne  x9, x24, Exit
       addi x22, x22, 1 
       beq  x0, x0, Loop    // unconditional이 되어야 한다.(다시 동일한 loop를 수행하도록.) 언제나 만족하는 일종의 trick을 쓴다.(x0를 보통 read only로 많이 써서 conventional하게 x0를 넣는데, 사실 꼭 x0일 필요는 없다.)
Exit: ...
```

---

## 추가: 변수 초기화 RISC-V instruction

만약 C code에서 어떤 변수를 초기화할 일이 있다면(register x5가 해당 변수를 저장한다고 하자)
